...
class ParseHentaiContentCommand extends Command
{
    protected $signature = 'parse:hentai-content {page?}';
    ...
    private function makeFile(ProxyService $proxy, Element $element, string $prefix = 'previews'): ?File
    {
        ...
        if (!$file) {
            $content = $proxy->through(timeout: 120)->get($link);
    ...
    public function handle(ProxyService $proxy, Parser $parser): void
    {
        ...
        $defaultSection = Section::firstOrCreate(['name' => 'Hentai']);

        progress(
            label: 'Parsing elements',
            steps: $tds,
            callback: function (Element $td, Progress $progress) use (&$videos, $proxy, $defaultSection) {
                $titleEl = $td
